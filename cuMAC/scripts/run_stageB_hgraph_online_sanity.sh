...
SANITY_SCRIPT="${ROOT_DIR}/training/stageb_hgraph/online_sanity.py"
...
OUT_DIR="${ROOT_DIR}/training/stageb_hgraph/runs/online_sanity"
EPISODES=1
EPISODE_HORIZON=400
MAX_STEPS=0
TOPOLOGY_SEED_MODE="fixed"
SEED_LIST=""
SOCKET_PATH="/tmp/cumac_stageb_hgraph.sock"
...
usage() {
    cat <<EOF
Run Stage-B sparse-entity graph sanity checks through the online bridge.
...
Online graph sanity options:
  ...
  --tb-err-ema-alpha <v>
  --print-every <n>
  --action-source <m>        noop | random_legal
  --random-blank-prob <v>    Used when action-source=random_legal
  --random-seed <n>          RNG seed for random_legal
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --random-blank-prob) RANDOM_BLANK_PROB="$2"; shift 2 ;;
...
if [[ ! -f "${SANITY_SCRIPT}" ]]; then
    echo "Missing graph sanity script: ${SANITY_SCRIPT}" >&2
    exit 1
fi

EXEC_MODE="$(echo "${EXEC_MODE}" | tr '[:upper:]' '[:lower:]')"
if [[ "${EXEC_MODE}" != "both" ]]; then
    echo "--exec-mode must be both for online bridge graph sanity." >&2
    exit 1
fi
...
echo "[Stage-B HGraph] action_source=${ACTION_SOURCE} random_blank_prob=${RANDOM_BLANK_PROB} random_seed=${RANDOM_SEED}"

CMD=(
    "${PYTHON_BIN}" "${SANITY_SCRIPT}"
    --sim-bin "${BIN}"
    --sim-args "${SIM_ARGS}"
    --sim-cwd "${SIM_CWD}"
    ...
    --random-blank-prob "${RANDOM_BLANK_PROB}"
...
echo "[Stage-B HGraph] running online graph sanity"
"${CMD[@]}"
