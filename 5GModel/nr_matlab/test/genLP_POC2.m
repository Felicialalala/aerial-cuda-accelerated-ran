...
compact_TC = [46, 48, 49, 50, 51, 59, 59.2, 59.3, 59.4, 59.5, 59.6, 60, 60.3, 60.4, 61, 62.3, 63.3, 65.1, 65.2, 65.3, 65.4, ...
                66.1, 66.2, 66.3, 66.4, 67, 67.1, 67.2, 67.3, 67.4, 67.5, 69, 69.1, 69.2, 69.3, 69.4, 69.5, 71, 73, 75, 79, 81.1, 81.2, 81.3, 81.4, 83.1, 83.2, 83.3, 83.4, 85];
...
CFG = {
      ...
      % patternNum,  nTvCell, patternType,              dlTvIdx1,                dlTvIdx2Delta,    ulTvIdx,         channels
        ...
        BFW_SRS_BIND_CFG = {
            % patternNum    srsTvIdx    srsSlotIdx    srsTvNameSlotIdx    bfwTvDlTvIdx    bfwDlSlotIdx   bfwTvUlTvIdx    bfwUlSlotIdx
...
if (patternType == "64TR_nrSim" || patternType == "64TR_nrSim_puschOnlyU" || patternType == "64TR_nrSim_624" || patternType == "64TR_nrSim_624_puschOnlyU" || patternType == "64TR_nrSim_638" || patternType == "64TR_nrSim_640") && ~isempty(bfw_srs_bind_cfg)
    for i = 1:size(bfw_srs_bind_cfg, 1)
        for idxCell = 1:nCell  % TODO assuming same SRS and BFW for all cells
            ...
            if bfwTvUlTvIdx ~= 0  % has binded DL BFW TV
...
if ~isempty(override_cfg)
    for i = 1:size(override_cfg, 1)
        ...
        % Only apply override if all cells in the override array exist in the current pattern
        % This prevents applying override to partial patterns (e.g., skip if override needs cells 0-14 but only 0-4 exist)
        max_cell_idx_needed = max(override_cell_idx_array);
        total_cells = sum(nCell);  % nCell can be a vector for some cases, so sum to get total
        if max_cell_idx_needed >= total_cells
            continue;  % Skip this entire override if not all required cells are in the current pattern
        end
...
cell_idx = 1;
for b = 1:bandwidth_num
    DL_TV_1 = cellstr(DL_TV{1, cell_idx}); %  testMAC increments by 1 for every new cell, use first TV in this bandwidth
    for idxCell = 1:nCell(b)
        % LP.Cell_Configs{cell_idx} = DL_TV{cell_idx};
...
if (ismember(patternNum, [69, 69.1, 69.2, 69.3, 69.4, 71, 73, 75, 77, 79, 81.1, 81.2, 81.3, 81.4, 83.1, 83.2, 83.3, 83.4, 85]))
...
for idxCell = 1:nCell
    for slot = 1:nSlot
        ...
        if ismember(slotInFrame, [5,6,15,16])
            ...
            if channels{slot}{1} == "Special!"                
                if slotInFrame == 6 && idxCell <= nCell / 2
                    LP.SCHED{slot}.config{idxCell}.type = channels{slot}(2:4);
                elseif slotInFrame == 16 && idxCell > nCell / 2
                    LP.SCHED{slot}.config{idxCell}.type = channels{slot}(2:4);
                else
                    LP.SCHED{slot}.config{idxCell}.type = channels{slot}(2:3);
                end
            end
        else
