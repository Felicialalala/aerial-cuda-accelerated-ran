...
% MINI TEST VECTOR 1: test_gen_dmrs_sym.h5
% (covers gen_dmrs_sym: previous 10 lines)
if SimCtrl.genTV.cuPHY_max
    saveToH5(SimCtrl.genTV.tvDirName, 'test_gen_dmrs_sym', ... 
        ... % Inputs:
...
% MINI TEST VECTOR 2: apply_ChEst_LS_main.h5
% (covers apply_ChEst_LS_main: previous 10 lines, but call is below)
if SimCtrl.genTV.cuPHY_max
    % Run just the LS ChEst (Note: this **exact** code is executed in the MMSE ChEst
    %                        i.e. function "pusch_ChEst_LS_delayEst_MMSE")
    % (VERIFIED: "H_LS_est" below matches "H_LS_est_save" in the original TV)
    [H_LS_est, R0_avg, R1_avg] = apply_ChEst_LS_main...
            (Xtf,puschTable,slotNumber,Nf,Nt,N_dmrs_id,...
            nl, portIdx, vec_scid, dmrsIdx{posDmrs}, Nt_dmrs, nPrb, startPrb, ...
            delta_f,maxLength, numDmrsCdmGrpsNoData, enableTfPrcd,...
            N_slot_frame, N_symb_slot, puschIdentity,...
    ...
    % Save all inputs and outputs for test_apply_ChEst_LS_main.m
    saveToH5(SimCtrl.genTV.tvDirName, 'test_apply_ChEst_LS_main', ...
        ... % Inputs:
        'Xtf', Xtf, ...
        'puschTable', 0, ... % this structure is NOT used and causes problems!
        'slotNumber', slotNumber, ...
        'Nf', Nf, ...
        'Nt', Nt, ...
        'N_dmrs_id', N_dmrs_id, ...
        ...
        'portIdx', portIdx, ...
        'vec_scid', vec_scid, ...
        'dmrsIdx', dmrsIdx{1}, ... % first dmrsIdx (AdditionalPosition = 0)
        'Nt_dmrs', Nt_dmrs, ...
        'nPrb', nPrb, ...
        'startPrb', startPrb, ...
        'delta_f', delta_f, ...
        ...
        'enableTfPrcd', uint8(enableTfPrcd), ... % LOGICAL NOT SUPPORTED
        'N_slot_frame', N_slot_frame, ...
        'N_symb_slot', N_symb_slot, ...
        'puschIdentity', puschIdentity, ...
        'groupOrSequenceHopping', groupOrSequenceHopping, ...
        'r_dmrs', r_dmrs, ...
        ... % Outputs:
        'H_LS_est', H_LS_est, ...
        'R0_avg', R0_avg, ...
...
% MINI TEST VECTOR 3: pusch_ChEst_LS_delayEst_MMSE.h5
% (covers pusch_ChEst_LS_delayEst_MMSE function: previous 65 lines, but just for SimCtrl.alg.ChEst_alg_selector = 1)
if SimCtrl.genTV.cuPHY_max
    % Save all inputs and outputs for test_pusch_ChEst_LS_delayEst_MMSE.m    
    saveToH5(SimCtrl.genTV.tvDirName, 'test_pusch_ChEst_LS_delayEst_MMSE', ...
        ... % Inputs:
        'Xtf', Xtf, ...
        'puschTable', 0, ... % the actual pusch table is not needed
        'slotNumber', slotNumber, ...
        'Nf', Nf, ...
        'Nt', Nt, ...
        'N_dmrs_id', N_dmrs_id, ...
        ...
        'carrier', carrier, ...
        'prgSize', prgSize, ...
        'enable_prg_chest', SimCtrl.bfw.enable_prg_chest, ...
        'r_dmrs', r_dmrs, ...
        ... % Outputs:
        'H_est', H_est{1}, ... % just first estimate, not per symbol!
        'delay_mean_microsec', delay_mean_microsec, ...
        'delay_spread_microsec', delay_spread_microsec ...%'dbg_chest', dbg_chest ...
    );
end
...
% SAVE VARIABLES FOR MAX TV
if SimCtrl.genTV.cuPHY_max
    nCov_before_shrinkage = nCov;
end
...
for idxUe = 1:nUe
    ...
    % SAVE VARIABLES FOR MAX TV
    if SimCtrl.genTV.cuPHY_max
        nl_offset_softdemapper = nl_offset;
    end
...
%                                                                              nPrb,nl,qam,nBitsHarq,N_id,n_rnti);
    ...
    % COMPLETE CUPHY MAX TEST VECTOR: TVnr_XXXX_cuPhyMax.h5
    if SimCtrl.genTV.cuPHY_max
        saveToH5(SimCtrl.genTV.tvDirName, [SimCtrl.genTV.TVname '_cuPhyMax'], ...
            ... % miniTV 01: GEN DMRS SYM
            'slotNumber', slotNumber, ...
            'Nf', Nf, ...
            'Nt', Nt, ...
            'N_dmrs_id', N_dmrs_id, ...
            'r_dmrs', r_dmrs, ...
            ... % miniTV 02: LS ChEst single call
            'Xtf', Xtf, ...
            'nl', nl, ...
            'portIdx', portIdx, ...
            'vec_scid', vec_scid, ...
            'dmrsIdx_first', dmrsIdx{1}, ...
            'Nt_dmrs', Nt_dmrs, ...
            'nPrb', nPrb, ...
            'startPrb', startPrb, ...
            'delta_f', delta_f, ...
            'maxLength', maxLength, ...
            'numDmrsCdmGrpsNoData', numDmrsCdmGrpsNoData, ... % also used for equalizer
            'enableTfPrcd_u8', uint8(enableTfPrcd), ...
            'N_slot_frame', N_slot_frame, ...
            'N_symb_slot', N_symb_slot, ...
            'puschIdentity', puschIdentity, ...
            'groupOrSequenceHopping', groupOrSequenceHopping, ...
            ...
            'R0_avg', R0_avg, ...
            'R1_avg', R1_avg, ...
            ... % miniTV 03: MMSE ChEst with delay and spread (selector==1)
            'dmrsIdx', dmrsIdx, ...
            'AdditionalPosition', AdditionalPosition, ...
            'carrier', carrier, ...
            'prgSize', prgSize, ...
            ...
            'H_est', H_est{1}, ...
            'delay_mean_microsec', delay_mean_microsec, ...
            'delay_spread_microsec', delay_spread_microsec, ...
            ... % miniTV 04-A: embed_dmrs_UL
            'Xtf_zero', zeros(size(Xtf)), ...
            'n_scid', n_scid, ...
            'symIdx_dmrs', symIdx_dmrs, ...
            'energy', PuschParams.energy, ...
            'grid_table', puschTable.grid_table, ...
            'fOCC_table', puschTable.fOCC_table, ...
            'tOCC_table', puschTable.tOCC_table, ...
            'enableTfPrcd', uint8(enableTfPrcd), ...
            'enablePrcdBf', 0, ...
            'PM_W', 0, ...
            'scrSeq', [], ...
            'XtfDmrs', XtfDmrs, ...
            ...
            'nAnt', nAnt, ...
            'r_tilde', r_tilde, ...
            'H_est_save', H_est_save, ...
            ... % miniTV 04-C: nCov calc
            'Rww_regularizer_val', Rww_regularizer_val, ...
            'nCov_before_shrinkage', nCov_before_shrinkage, ...
            'tmp_noiseVar', tmp_noiseVar, ...
            'num_samples_nCov', num_samples_nCov, ...
            ... % miniTV 04-D: shrinkage
            'enable_nCov_shrinkage', enable_nCov_shrinkage, ...
            'nCov_shrinkage_method', nCov_shrinkage_method, ...
            'SimCtrl_subslot_proc_option', SimCtrl.subslot_proc_option, ...
            'nCov', nCov, ...
            ... % miniTV 05: noiseVar/RSRP/SINR
            'noiseVar', noiseVar, ...
            'N0_ref', N0_ref, ...
            ...
            'sinrdB', sinrdB, ...
            ... % miniTV 06: Equalizer
            'noiseIntfCov', noiseIntfCov, ...
            'posDmrs', posDmrs, ...
            'W', W{1}, ...
            'cfo_est_symbols_layers', cfo_est_symbols_layers, ...
            'to_est', to_est, ...
            'enableCfoCorrection', enableCfoCorrection, ...
            'enableToCorrection', enableToCorrection, ...
            'TdiMode', TdiMode, ...
            'X_est', X_est, ...
            'Ree', Ree, ...
            'symIdx_data', symIdx_data, ...
            ... % miniTV 07: Post-EQ SINR
            'nUe', nUe, ...
            'postEqNoiseVardB', postEqNoiseVardB, ...
            'postEqSinrdB', postEqSinrdB, ...
            ... % miniTV 08: Soft demapper
            'Nt_data', Nt_data, ...
            'nl_offset_softdemapper', nl_offset_softdemapper, ...
            'qam', qam, ...
            'd_qpsk', puschTable.d_qpsk, ...
            'd_qam16', puschTable.d_qam16, ...
            'd_qam64', puschTable.d_qam64, ...
            'd_qam256', puschTable.d_qam256, ...
            'LLRseq', LLRseq, ...
            'LLR_demap', LLR_demap, ...
            ... % miniTV 09: De-scrambling bits
            'N_id', N_id, ...
            'n_rnti', n_rnti, ...
            ... % miniTV 10: Derate match
            'LLR_descr', LLR_descr, ...
            'BGN', BGN, ...
            'C', C, ...
            'K', K, ...
            ...
            'derateCbs', derateCbs, ...
            'nV_parity', nV_parity, ...
            'derateCbsIndices', derateCbsIndices, ...
            'derateCbsIndicesSizes', derateCbsIndicesSizes, ...
            ... % miniTV 11: LDPC/CRC decode (include tables)
            'i_LS', i_LS, ...
            'CRC', uint8(CRC), ...
            'maxNumItr_CBs', maxNumItr_CBs, ...
            'crcTable_24A', puschTable.crcTable_24A, ...
            'crcTable_24B', puschTable.crcTable_24B, ...
            'crcTable_24C', puschTable.crcTable_24C, ...
            'crcTable_16', puschTable.crcTable_16, ...
            'crcTable_11', puschTable.crcTable_11, ...
            'BG1_NeighborIndicies', puschTable.BG1_NeighborIndicies, ...
            'BG2_NeighborIndicies', puschTable.BG2_NeighborIndicies, ...
            'BG1_numNeighbors', puschTable.BG1_numNeighbors, ...
            'BG2_numNeighbors', puschTable.BG2_numNeighbors, ...
            'BG1_NeighborPermutations_LS1', puschTable.BG1_NeighborPermutations_LS1, ...
            'BG1_NeighborPermutations_LS2', puschTable.BG1_NeighborPermutations_LS2, ...
            'BG1_NeighborPermutations_LS3', puschTable.BG1_NeighborPermutations_LS3, ...
            'BG1_NeighborPermutations_LS4', puschTable.BG1_NeighborPermutations_LS4, ...
            'BG1_NeighborPermutations_LS5', puschTable.BG1_NeighborPermutations_LS5, ...
            'BG1_NeighborPermutations_LS6', puschTable.BG1_NeighborPermutations_LS6, ...
            'BG1_NeighborPermutations_LS7', puschTable.BG1_NeighborPermutations_LS7, ...
            'BG1_NeighborPermutations_LS8', puschTable.BG1_NeighborPermutations_LS8, ...
            'BG2_NeighborPermutations_LS1', puschTable.BG2_NeighborPermutations_LS1, ...
            'BG2_NeighborPermutations_LS2', puschTable.BG2_NeighborPermutations_LS2, ...
            'BG2_NeighborPermutations_LS3', puschTable.BG2_NeighborPermutations_LS3, ...
            'BG2_NeighborPermutations_LS4', puschTable.BG2_NeighborPermutations_LS4, ...
            'BG2_NeighborPermutations_LS5', puschTable.BG2_NeighborPermutations_LS5, ...
            'BG2_NeighborPermutations_LS6', puschTable.BG2_NeighborPermutations_LS6, ...
            'BG2_NeighborPermutations_LS7', puschTable.BG2_NeighborPermutations_LS7, ...
            'BG2_NeighborPermutations_LS8', puschTable.BG2_NeighborPermutations_LS8, ...
            'TbCbs_est', TbCbs_est, ...
            'numItr', numItr, ...
            'badItrCnt', badItrCnt, ...
            'TbCrc_est', TbCrc_est, ...
            ...
            'tbErr', tbErr, ...
            ... % miniTV 12: RSSI Est.
            'dmrsRssiReportedDb', dmrsRssiReportedDb, ...
            'dmrsRssiReportedDb_ehq', dmrsRssiReportedDb_ehq, ...
            'dmrsRssiDb', dmrsRssiDb ...
        );
    end
...
elseif strcmp(SimCtrl.alg.LDPC_DMI_method,'ML')  
    gnb_pars.ldpcMaxNumItrAlgIdx = uint8(2); % placeholder for machine learning method
end
