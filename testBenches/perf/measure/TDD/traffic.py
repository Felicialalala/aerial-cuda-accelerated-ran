...
def traffic_avg(args, vectors, testcases, filenames):
    ...
    # Load DL slot config from JSON file with
    slotConfig = json.load(open("measure/TDD/slotConfig.json"))

    if args.pattern == "dddsu":
        from .DDDSU.traffic.generate import run

        run(args, vectors, testcases, filenames, slotConfig[args.pattern])
    else:
