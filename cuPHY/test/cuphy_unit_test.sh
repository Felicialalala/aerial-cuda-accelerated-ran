...
function usage {
   echo './cuphy_unit_test.sh <Base_TV_path> <GPU> <PUSCH_CB_ERROR_CHECK> <RUN_COMPONENT_TESTS> <RUN_COMPUTE_SANITIZER> <PYTHON_BIN> <UNIT_TEST_BLOCK_TO_RUN_FIRST> [-b <BUILD_DIR>] [--srs_bfw]'
   echo ''
   echo 'Alternative usage with optional flags:'
   echo './cuphy_unit_test.sh [-t <Base_TV_path>] [-g <GPU>] [-c <PUSCH_CB_ERROR_CHECK>] [-m <RUN_COMPONENT_TESTS>] [-s <RUN_COMPUTE_SANITIZER>] [-p <PYTHON_BIN>] [-f <UNIT_TEST_BLOCK_TO_RUN_FIRST>] [-b <BUILD_DIR>] [-o <ONLY_BLOCK>] [--srs_bfw]'
   echo ''
   echo '   Parameters            Default Value    Description'
   echo '   Base_TV_path          ./               Path to test vectors'
   echo '     -t, --tv-path                        Alternative flag for Base_TV_path'
   echo '   GPU:                   0               CUDA device to execute on'
   echo '     -g, --gpu                            Alternative flag for GPU'
   echo '   PUSCH_CB_ERROR_CHECK   1 (enabled)     flag to enable (1) or disable (0) PUSCH CB Error check'
   echo '     -c, --cb-error-check                 Alternative flag for PUSCH_CB_ERROR_CHECK'
   echo '   RUN_COMPONENT_TESTS    0 (disabled)    flag to enable (1) or disable (0) Component level tests'
   echo '     -m, --component-tests                Alternative flag for RUN_COMPONENT_TESTS'
   ...
   echo '     -s, --sanitizer                      Alternative flag for RUN_COMPUTE_SANITIZER'
   echo '   PYTHON_BIN             python3         python call to execute scripts with'
   echo '     -p, --python                         Alternative flag for PYTHON_BIN'
   echo '   UNIT_TEST_BLOCK_TO_RUN_FIRST 0         test block id to run first'
   echo '                                           PUSCH           = 0 (default)'
   echo '                                           PDSCH           = 1 '
   echo '                                           PDCCH           = 2 '
   echo '                                           PUCCH           = 3 '
   echo '                                           SSB             = 4 '
   ...
   echo '                                           Simplex         = 12 '
   echo '                                           deRateMatch     = 13 '
   echo '                                           Component Tests = 14 '
   echo '                                           CSI-RS RX       = 15 '
   echo '     -f, --first-block                    Alternative flag for UNIT_TEST_BLOCK_TO_RUN_FIRST'
   echo '   -b <BUILD_DIR>                         Custom build directory path (default: $CUPHY_ROOT/../build/cuPHY)'
   echo '   -o, --only-block                       Run only the specified test block (same indices as above)'
   echo '   --srs_bfw                              flag to run only SRS+BFW tests'
   exit
}
        ...
        for bfw_tv in $(find $BASE_TV_PATH -name "TVnr_*_BFW_gNB_CUPHY_*.h5"); do
            ...
            if [ -n "$srs_tv_nums" ]; then
                ...
                for srs_tv_num in $srs_tv_nums; do
                    # Try both naming patterns for SRS TV
                    if ([ $srs_tv_num -ge 8000 ] && [ $srs_tv_num -le 8999 ]) || [ $srs_tv_num -le 999 ]; then
                        srs_tv=$(find $BASE_TV_PATH -name "TVnr_$(printf '%04d' $srs_tv_num)_SRS_gNB_CUPHY_*.h5")
                    else
                        srs_tv=$(find $BASE_TV_PATH -name "TVnr_ULMIX_$(printf '%04d' $srs_tv_num)_SRS_gNB_CUPHY_*.h5")
                    fi
...
CUPHY_ROOT=$(builtin cd $TESTDIR/..;pwd)
