...
#define TAG (NVLOG_TAG_BASE_CUPHY_DRIVER + 33) // "DRV.DLBFW"
    ...
    PhyChannel(_pdh, _gDev, 0, _s_channel, _mpsCtx)
{
    ...
    mf.init(_pdh, std::string("PhyDlBfw"), sizeof(PhyDlBfwAggr));
    cuphyMf.init(_pdh, std::string("cuphyDlBfwTx"), 0);

    channel_type = slot_command_api::channel_type::BFW; //TODO :Assign DLBFW equivalent
    channel_name.assign("DL_BFW");//TODO :Assign DLBFW equivalent
    ...
    {
        #if 0
        dlBfwChEstBuffInfo[i] = std::move(cuphy::tensor_device(nullptr, CUPHY_C_32F, CV_NUM_PRBG, 
                                                        CV_NUM_GNB_ANT,
                                                        CV_NUM_UE_LAYER, 
                                                        cuphy::tensor_flags::align_tight));
        data_in.pChEstInfo[i].tChEstBuffer.desc = dlBfwChEstBuffInfo[i].desc().handle();
        #endif
...
int PhyDlBfwAggr::createPhyObj()
{
    ...
    //TODO : Remove hardcoding and read params from global DBs
    stat_params.lambda = 0;
    stat_params.nMaxGnbAnt = CV_NUM_GNB_ANT;
    stat_params.nMaxPrbGrps = slot_command_api::MAX_NUM_PRGS_DBF;
    stat_params.nMaxUeGrps = slot_command_api::MAX_PDSCH_UE_GROUPS;
    stat_params.nMaxTotalLayers = slot_command_api::MAX_PDSCH_UE_GROUPS * CUPHY_BFW_COEF_COMP_N_MAX_LAYERS_PER_USER_GRP;
    ...
    NVLOGD_FMT(TAG, "PhyDlBfwAggr::bfwPowerNormAlg_selector: {}", stat_params.bfwPowerNormAlg_selector);

    stat_params.pDbg = &dlBfwDbgPrms;
    stat_params.pStatDbg = &dlBfwStatDbgPrms;
    stat_params.pOutInfo = &cuphy_tracker;

#ifdef DLBFW_H5DUMP
    //std::string dbg_output_file = std::string("DLBFW_Debug") + std::to_string(id) + std::string(".h5");
    ...
    std::string stat_dbg_output_file = std::string("DLBFW_Stat_Debug") + std::to_string(id) + std::string(".h5");
...
slot_command_api::bfw_params* PhyDlBfwAggr::getDynParams()
{
    return aggr_slot_params->cgcmd->bfw.get();
}

int PhyDlBfwAggr::setup(std::vector<Cell *>& aggr_cell_list)
{
    PhyDriverCtx* pdctx = StaticConversion<PhyDriverCtx>(pdh).get();
    cuphyStatus_t status;
    ...
    NVLOGD_FMT(TAG, "DL prevUeGrpChEstInfoBufIdx={}", pparms->prevUeGrpChEstInfoBufIdx);
    ...
    dyn_params.pDynDbg = &dlBfwDynDbgPrms;
    ...
    NVLOGD_FMT(TAG, "DL PhyBfwAggr{} SFN {}.{} setup", this_id, si->sfn_, si->slot_);
    return 0;
}

int PhyDlBfwAggr::run()
{
    cuphyStatus_t status;
    int ret=0;
    ...
    struct slot_command_api::slot_indication* si = aggr_slot_params->si;
    NVLOGD_FMT(TAG, "DL PhyBfwAggr{} SFN {}.{} run", this_id, si->sfn_, si->slot_);

    return ret;
}

int PhyDlBfwAggr::callback()
{
    struct slot_command_api::slot_indication* si = aggr_slot_params->si;
    NVLOGD_FMT(TAG,"DLBFW pipeline {} SFN {}.{} callback entered",this_id, si->sfn_, si->slot_);
    return 0;
}


int PhyDlBfwAggr::validate() {
    int ret = 0;
#ifdef DLBFW_H5DUMP
        bool triggerH5Dump = true;
        if(triggerH5Dump)
        {
            NVLOGC_FMT(TAG, "SFN {}.{} Generating H5 Debug DLBFW file {}", aggr_slot_params->si->sfn_, aggr_slot_params->si->slot_, std::to_string(id).c_str());
