...
Bootstrap += baseimage(image='nvcr.io/nvidia/cuda:12.9.1-devel-ubuntu22.04', _arch=cpu_target, _distro='ubuntu22', _as='bootstrap')
...
# GDRCopy
Bootstrap += gdrcopy(version='2.5', ldconfig=True)
...
Externals += generic_cmake(
    cmake_opts=['-DCMAKE_CXX_STANDARD=17','-DBUILD_SHARED_LIBS=ON','-DCMAKE_BUILD_TYPE=Release','-DgRPC_INSTALL=ON'],
...
Externals += shell(commands=[
    'wget -q https://developer.nvidia.com/downloads/compute/cuFFTDx/redist/cuFFTDx/cuda12/nvidia-mathdx-25.06.1-cuda12.tar.gz',
    'tar xzf nvidia-mathdx-25.06.1-cuda12.tar.gz -C /usr/local --strip-components=1',
    'rm nvidia-mathdx-25.06.1-cuda12.tar.gz',
    ])
...
Base += raw(docker=f'ADD https://www.mellanox.com/downloads/DOCA/DOCA_v3.0.0/host/doca-host_3.0.0-058000-25.04-ubuntu2204_{TARGETARCH}.deb /tmp/doca-host/')
repo_dir = f'/usr/share/doca-host-3.0.0-058000-25.04-ubuntu2204/repo'
...
if AERIAL_GPU_TYPE == "gl4":
    Base += copy(src=f'gl4/doca-sdk-gpunetio_3.0.0058-1_{TARGETARCH}.deb', dest=f'{repo_dir}/pool/')
    Base += copy(src=f'gl4/libdoca-sdk-gpunetio-dev_3.0.0058-1_{TARGETARCH}.deb', dest=f'{repo_dir}/pool/')
    Base += copy(src=f'gl4/doca-sdk-eth_3.0.0058-1_{TARGETARCH}.deb', dest=f'{repo_dir}/pool/')
    Base += copy(src=f'gl4/libdoca-sdk-eth-dev_3.0.0058-1_{TARGETARCH}.deb', dest=f'{repo_dir}/pool/')
    deb_list = [
        'doca-sdk-gpunetio',
    ...
    Base += copy(src=f'doca-sdk-gpunetio_3.0.0058-1_{TARGETARCH}.deb', dest=f'{repo_dir}/pool/')
    Base += copy(src=f'libdoca-sdk-gpunetio-dev_3.0.0058-1_{TARGETARCH}.deb', dest=f'{repo_dir}/pool/')
    deb_list = [
        'doca-sdk-gpunetio',
        'libdoca-sdk-gpunetio-dev',
        'libdoca-sdk-argp-dev',
