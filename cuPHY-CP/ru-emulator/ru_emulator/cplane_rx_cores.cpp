...
void RU_Emulator::increment_section_rx_counter(oran_c_plane_info_t& c_plane_info, uint16_t cell_index, int sym)
{
    if(c_plane_info.valid_eaxcId)
    {
        for(int sec = 0; sec < c_plane_info.numberOfSections; ++sec)
        {
            ...
            int ul_flows = c_plane_info.channel_type == ul_channel::SRS ? cell_configs[cell_index].num_valid_SRS_flows : cell_configs[cell_index].num_ul_flows;
            int prb_num = c_plane_info.channel_type == ul_channel::SRS ? tv_info.fss_numPrb[c_plane_info.fss.frameId][counter_index] : tv_info.numPrb;
            int expected_prbs = prb_num * ul_flows;
            if (opt_enable_mmimo && c_plane_info.channel_type != ul_channel::SRS)
            {
...
bool RU_Emulator::validate_modulation_compression(const oran_c_plane_info_t& c_plane_info, const oran_c_plane_section_info_t& section_info, int cell_index)
{
    ...
    const auto reMask = section_info.reMask;
    bool found = false;
            ...
            tv_obj_p->launch_pattern[c_plane_info.launch_pattern_slot].find(cell_index) != tv_obj_p->launch_pattern[c_plane_info.launch_pattern_slot].end())
        {
            ...
            for (int sym = c_plane_info.startSym; sym < c_plane_info.startSym + section_info.numSymbol; sym++)
            {
                for (int msg_port_idx = portIdx;; msg_port_idx += cell_configs[cell_index].num_dl_flows)
                {
                    ...
                    if (hdr.find(sym) != hdr.end() &&
                        hdr[sym].find(msg_port_idx) != hdr[sym].end() &&
                        hdr[sym][msg_port_idx].find(reMask) != hdr[sym][msg_port_idx].end())
                    {
                        for (auto &e : hdr[sym][msg_port_idx][reMask])
                        {
 ...
 * @param[in] cell_index Cell identifier for the validation
 */
void RU_Emulator::validate_remask(oran_c_plane_info_t& c_plane_info, int cell_index)
{
    bool is_pdsch_included = c_plane_channel_type_checking(c_plane_info, cell_index, pdsch_object);
    ...
    if (opt_enable_mmimo)
    {
        ...
        if (!is_csirs_included && is_pdsch_included)
        {
            ...
            if (c_plane_info.section_infos_size > 1)
            {
                ...
                if (pdsch_remask_mismatch)
                {
                    ...
                    {
                        sleep(1);
                        do_throw(sb() << "PDSCH reMask verification failed... ");
                    }
                }
            }
        }
        else if (is_csirs_included && is_pdsch_included)
        {
            ...
            if (c_plane_info.section_infos_size > 1)
            {
                ...
                for (int sec_idx = 0; sec_idx < c_plane_info.section_infos_size; sec_idx++)
                {
                    ...
                    if (csirs_section)
                    {
                        ...
                        if(c_plane_info.section_infos[sec_idx].reMask != expected_reMask)
                        {
                            ...
                            invalid_sections[1][csirs_remask_mismatch][2] = c_plane_info.section_infos[sec_idx].reMask;
                ...
                if (csirs_remask_mismatch || pdsch_remask_mismatch || missing_pdsch_section)
                {
                    ...
                    {
                        sleep(1);
                        do_throw(sb() << "CSI_RS&PDSCH reMask verification failed... ");
                    }
                }
            }
        }
        else if (is_csirs_included && !is_pdsch_included)
        {
            ...
            if (csirs_remask_mismatch)
            {
                ...
                {
                    sleep(1);
                    do_throw(sb() << "CSI_RS reMask verification failed... ");
                }
            }
        }
    }
    else
    {
        ...
        if (is_pdsch_included)
        {
            ...
            if (c_plane_info.section_infos_size > 1)
            {
                for (int sec_idx = 0; sec_idx < c_plane_info.section_infos_size; sec_idx += 2)
                {
                    ...
                    if (c_plane_info.section_infos[sec_idx + 1].reMask != expected_reMask)
                    {
                        csirs_remask_mismatch++;
        ...
        {
            // CSIRS only scenario (mMIMO disabled)
            for (int sec_idx = 0; sec_idx < c_plane_info.section_infos_size; sec_idx++)
            {
                int remask_idx = remask_idx_base + c_plane_info.section_infos[sec_idx].startPrbc;
                expected_reMask = tv_info.csirsREMaskArrayTRSNZP[remask_idx];
                                     ...
                                     FssPdschPrbSeenArray& fss_pdsch_prb_seen)
{
    ...
    if(opt_csirs_validation && cell_configs[cell_index].dl_comp_meth != aerial_fh::UserDataCompressionMethod::MODULATION_COMPRESSION)
    {
        //Skip for modComp as remask is already used as part of the info to identify corresponding modcomp MSGs
        bool remask_validation = c_plane_channel_type_checking(c_plane_info, cell_index, csirs_object);
        if (remask_validation)
        {
            validate_remask(c_plane_info, cell_index);
        }
    }
...
int RU_Emulator::handle_sect1_c_plane(oran_c_plane_info_t &c_plane_info, uint16_t cell_index, tx_symbol_timers &timers, aerial_fh::TxqHandle *txqs, aerial_fh::TxRequestHandle* tx_request, int sym, perf_metrics::PerfMetricsAccumulator* profiler)
{
    ...
    // for (int i = 0; i < ORAN_ALL_SYMBOLS; ++i)
    {
        ...
        for (int sec = 0; sec < c_plane_info.numberOfSections; ++sec)
        {
            ...
            {
                tx_symbol_info.numPrbc = cell_configs[cell_index].ru_type == ru_type::SINGLE_SECT_MODE ? cell_configs[cell_index].ulGridSize : std::min((int)c_plane_info.tv_object->tv_info[section_info.tv_index].numPrb, cell_configs[cell_index].ulGridSize);
            }
            ...
            tx_symbol(c_plane_info.tv_object->slots[loaded_idx][section_info.tv_index], tx_symbol_info, c_plane_info.tv_object->blank_prbs.get(), timers, ORAN_CMSG_SECTION_TYPE_1, uplane_msg);
            ++num_tx_pkts;
...
void RU_Emulator::find_channel_type_for_each_section(oran_c_plane_info_t& c_plane_info, uint16_t cell_index) {
    for (int sec = 0; sec < c_plane_info.numberOfSections; ++sec)
    {
        ...
        else if(opt_srs_enabled && prb_range_matching(c_plane_info, section_info, cell_index, srs_object))
        {
            section_info.tv_object = &srs_object;
            section_info.channel_type = ul_channel::SRS;
            c_plane_info.tv_object = &srs_object;
...
inline void RU_Emulator::dynamic_beamid_validation(oran_c_plane_info_t &c_plane_info, int cell_index)
{
    ...
    if (eaxcId_index < 0 || slot_3gpp < 0 || slot_3gpp >= launch_pattern_slot_size)
    {
...
void RU_Emulator::parse_c_plane(oran_c_plane_info_t& c_plane_info, int nb_rx, int index_rx, uint64_t rte_rx_time, uint8_t* mbuf_payload, int cell_index)
{
    ...
    if(c_plane_info.dir == DIRECTION_UPLINK)
    {
        if (c_plane_info.section_type == ORAN_CMSG_SECTION_TYPE_1)
        {
            // Enable this branch when mixed channels in one packet are supported. For now we disable it to improve UL perf
            // if (opt_multi_section_ul == RE_ENABLED)
            // {
            //     find_channel_type_for_each_section(c_plane_info, cell_index);
            // }
            // else
            {
                find_channel_type(c_plane_info, cell_index);
            }
        }
        else
        {
...
void* RU_Emulator::cplane_core(void *arg) {
    ...
    while (!check_force_quit()) {
        ...
        for(int index_rx = 0; index_rx < nb_rx; ++index_rx)
        {
            ...
            parse_c_plane(c_plane_info, nb_rx, index_rx, rte_rx_time, mbuf_payload, cell_index);
