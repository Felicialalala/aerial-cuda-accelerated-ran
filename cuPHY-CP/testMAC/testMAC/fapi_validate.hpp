...
#ifndef _FAPI_VALIDATE_HPP_
#define _FAPI_VALIDATE_HPP_
#include <random>
...
public:
    ...
    fapi_validate();
    fapi_validate(int _enable, int _log_opt);
    ~fapi_validate();
    ...
    int pdu_start(int pdu_id, int channel, fapi_req_t* req);

    // Call after one PDU ended
    int pdu_ended(int pdu_id, int ind_id);

    // Call after one FAPI message ended
    int msg_ended();

    void set_fapi_req(fapi_req_t* req)
    {
        fapi_req = req;
    }

    fapi_req_t* get_fapi_req()
    {
        return fapi_req;
    }
    ...
    void log_value(int level, const char* name1, const char* name2, short2* val1, float2* val2, uint len);
    ...
    int check_complex_approx_ratio(int level, const char* name1, const char* name2, float2* val1, float2* val2, uint len, float tolerance = 0.001f, uint num_samples = 100)
    {
        if (len == 0 || num_samples == 0)
        {
            return 0; // No comparison needed if length or samples are zero
        }

        // use FP32 in comparison
        float diff_mag_sq = 0.0f;
        float val2_mag_sq = 0.0f;
        float ratioDiff = 0.0f;

        if(num_samples >= len) // Full validation
        {
            ...
            for(uint idx = 0; idx < len; idx++)
            {
                ...
                diff_mag_sq += (diff.x * diff.x + diff.y * diff.y);
                val2_mag_sq += (val2[idx].x * val2[idx].x + val2[idx].y * val2[idx].y);
            }
        }
        else // Partial random validation
        {
            num_samples = std::min(num_samples, len);
            // Fixed seed
            unsigned                            seed = 49;
            std::default_random_engine          generator(seed);
            std::uniform_int_distribution<uint> distribution(0, len - 1);
        ...
        ratioDiff = diff_mag_sq / val2_mag_sq;
    ...
    int check_complex_integer_approx_ratio(int level, const char* name1, const char* name2, short2* val1, float2* val2, uint len, float tolerance = 0.001f, uint num_samples = 100)
    {
        if (len == 0 || num_samples == 0)
        {
            return 0; // No comparison needed if length or samples are zero
        ...
        if(num_samples >= len) // Full validation
        {
            ...
            for(uint idx = 0; idx < len; idx++)
            {
                float2 diff{(float)(val1[idx].x) - val2[idx].x, (float)(val1[idx].y) - val2[idx].y};
...
#define FAPI_VALIDATE_U8_WARN(vald, val1, val2, ...) \
    ((vald)->check_value<uint8_t>(VALD_ENABLE_WARN, #val1, #val2, (val1), (val2), ##__VA_ARGS__))

// Validate int8_t value
#define FAPI_VALIDATE_I8_ERR(vald, val1, val2, ...) \
    ((vald)->check_value<int8_t>(VALD_ENABLE_ERR, #val1, #val2, (val1), (val2), ##__VA_ARGS__))
#define FAPI_VALIDATE_I8_WARN(vald, val1, val2, ...) \
    ((vald)->check_value<int8_t>(VALD_ENABLE_WARN, #val1, #val2, (val1), (val2), ##__VA_ARGS__))

// Validate float value
#define FAPI_VALIDATE_FLOAT_ERR(vald, val1, val2, ...) \
    ((vald)->check_value<float>(VALD_ENABLE_ERR, #val1, #val2, (val1), (val2), ##__VA_ARGS__))
#define FAPI_VALIDATE_FLOAT_WARN(vald, val1, val2, ...) \
    ((vald)->check_value<float>(VALD_ENABLE_WARN, #val1, #val2, (val1), (val2), ##__VA_ARGS__))

// Validate float value
#define FAPI_VALIDATE_FLOAT_APPROX_ERR(vald, val1, val2, ...) \
    ((vald)->check_approx_value(VALD_ENABLE_ERR, #val1, #val2, (val1), (val2), ##__VA_ARGS__))
#define FAPI_VALIDATE_FLOAT_APPROX_WARN(vald, val1, val2, ...) \
    ((vald)->check_approx_value(VALD_ENABLE_WARN, #val1, #val2, (val1), (val2), ##__VA_ARGS__))

// Validate payload bytes
#define FAPI_VALIDATE_BYTES_ERR(vald, buf1, buf2, nbytes) \
    (vald)->check_bytes(VALD_ENABLE_ERR, #buf1, #buf2, (buf1), (buf2), nbytes)
#define FAPI_VALIDATE_BYTES_WARN(vald, buf1, buf2, nbytes) \
    (vald)->check_bytes(VALD_ENABLE_WARN, #buf1, #buf2, (buf1), (buf2), nbytes)

// Validate complex value
#define FAPI_VALIDATE_COMPLEX_APPROX_RATIO_ERR(vald, val1, val2, len, ...) \
    ((vald)->check_complex_approx_ratio(VALD_ENABLE_ERR, #val1, #val2, (val1), (val2), len, ##__VA_ARGS__))
#define FAPI_VALIDATE_COMPLEX_APPROX_RATIO_WARN(vald, val1, val2, ...) \
    ((vald)->check_complex_approx_ratio(VALD_ENABLE_WARN, #val1, #val2, (val1), (val2), len, ##__VA_ARGS__))
    
// Validate complex normalized value
#define FAPI_VALIDATE_COMPLEX_INTEGER_APPROX_RATIO_ERR(vald, val1, val2, len, ...) \
    ((vald)->check_complex_integer_approx_ratio(VALD_ENABLE_ERR, #val1, #val2, (val1), (val2), len, ##__VA_ARGS__))
#define FAPI_VALIDATE_COMPLEX_INTEGER_APPROX_RATIO_WARN(vald, val1, val2, ...) \
    ((vald)->check_complex_integer_approx_ratio(VALD_ENABLE_WARN, #val1, #val2, (val1), (val2), len, ##__VA_ARGS__))

#endif /* _FAPI_VALIDATE_HPP_ */
